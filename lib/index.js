...
const showProgress = (onComplete, flags) => {
  const yourName = flags.name ? flags.name : "あなた";
  console.log("\nloading 2021...");
  ...
  let timer = setInterval(function () {
    ...
    if (value >= b1.getTotal()) {
      ...
      setTimeout(() => {
        ...
        spinner.start(`2021 年は ${yourName} にとって ...`);
        setTimeout(() => {
          spinner.succeed(
            `2021 年は ${yourName} にとって「素晴らしい一年」になるでしょう！👨‍💻`
          );
...
const showHappyNewYear2021 = async ({ random, name: yourName }) => {
  ...
  setTimeout(() => {
    lolcatjs.fromString(
      box(
        cowsay.say({
          text: `\n${yourName ? yourName + ", " : ""}Happy New Year 2021!\n`,
          f: cow ? cow.slice(0, -4) : "default",
        }) + "\n\n\nfrom @takapiro99 (​https://github.com/takapiro99​)"
      ) +
        message +
        "\n"
    );
